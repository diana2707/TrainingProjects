...
Console.WriteLine($"Account 1: Account # {account1.AccountNumber}, " +
    $"type {account1.AccountType}, balance {account1.Balance}, " +
    $"rate {BankAccount.InterestRate}, customer ID {account1.CustomerId}");

Console.WriteLine($"Account 2: Account # {account2.AccountNumber}, " +
    $"type {account2.AccountType}, balance {account2.Balance}, " +
    $"rate {BankAccount.InterestRate}, customer ID {account2.CustomerId}");
Console.WriteLine($"Account 3: Account # {account3.AccountNumber}, " +
    $"type {account3.AccountType}, balance {account3.Balance}, " +
...
Console.WriteLine($"Account 2 after withdrawal: Balance = {account2.Balance.ToString("C", CultureInfo.CurrentCulture)}, " +
    $"Withdrawal successful: {withdrawSuccess}");
...
Console.WriteLine($"Account 3 after transfer: Balance = {account3.Balance.ToString("C", CultureInfo.CurrentCulture)}, " +
...
Console.WriteLine($"Compound interest on account1 balance of {principal.ToString("C", CultureInfo.CurrentCulture)} " +
    $"at {rate * 100:F2}% for {time} year: {compoundInterest.ToString("C", CultureInfo.CurrentCulture)}");
...
Console.WriteLine($"Transaction fee for a {transactionAmount.ToString("C", CultureInfo.CurrentCulture)} " +
    $"wire transfer at a {BankAccount.TransactionRate * 100:F2}% rate "+
    $"and a max fee of {BankAccount.MaxTransactionFee.ToString("C", CultureInfo.CurrentCulture)} "+
    $"is: {transactionFee.ToString("C", CultureInfo.CurrentCulture)}");
...
Console.WriteLine($"Overdraft fee for an account that's " +
    $"{overdrawnAmount.ToString("C", CultureInfo.CurrentCulture)} " +
    $"overdrawn, using a penalty rate of {BankAccount.OverdraftRate * 100:F2}% "+
    $"and a max fee of {BankAccount.MaxOverdraftFee.ToString("C", CultureInfo.CurrentCulture)} "+
    $"is: {overdraftFee.ToString("C", CultureInfo.CurrentCulture)}");
...
Console.WriteLine($"The foreign currency received after exchanging " +
    $" {originalCurrencyProvided.ToString("C", CultureInfo.CurrentCulture)} " +
    $"at an exchange rate of {currentExchangeRate:F2} "+
    $"is: {foreignCurrencyReceived.ToString("C", CultureInfo.CurrentCulture)}");
...
Console.WriteLine($"Account 1 after applying interest: " +
    $"Balance = {account1.Balance.ToString("C", CultureInfo.CurrentCulture)}, " +
    $"Interest Rate = {BankAccount.InterestRate:P2}, " +
    $"Time Period = {timePeriodYears:F2} years");
...
Console.WriteLine($"Account 2 after requesting cashier's check: Received cashier's check: {receivedCashiersCheck}, " +
    $"Balance = {account2.Balance.ToString("C", CultureInfo.CurrentCulture)}, "+
    $"Transaction Amount = {checkAmount.ToString("C", CultureInfo.CurrentCulture)}, "+
    $"Transaction Fee Rate = {BankAccount.TransactionRate:P2}, "+
    $"Max Transaction Fee = {BankAccount.MaxTransactionFee.ToString("C", CultureInfo.CurrentCulture)}");

// Apply refund
Console.WriteLine("Applying refund to Account 3...");
double refundAmount = 50;
account3.ApplyRefund(refundAmount);
Console.WriteLine($"Account 3 after applying refund: " +
    $"Balance = {account3.Balance.ToString("C", CultureInfo.CurrentCulture)}, " +
    $"Refund Amount = {refundAmount.ToString("C", CultureInfo.CurrentCulture)}");
...
Console.WriteLine($"Account 8: Account # {account8.AccountNumber}, " +
    $"type {account8.AccountType}, " +
    $"balance {account8.Balance}, " +
    $"rate {BankAccount.InterestRate}, " +
    $"customer ID {account8.CustomerId}");

// Using copy constructor
BankAccount account9 = new BankAccount(account4);
Console.WriteLine($"Account 9 (copy of account4): " +
    $"Account # {account9.AccountNumber}, " +
    $"type {account9.AccountType}, " +
    $"balance {account9.Balance}, " +
    $"rate {BankAccount.InterestRate}, " +
    $"customer ID {account9.CustomerId}");
